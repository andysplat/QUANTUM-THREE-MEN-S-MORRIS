...
next_player(Color1, Color2) :- Color1 = white, Color2 = black; Color1 = black, Color2 = white.
...
valid_pairs(State, Color, Pairs) :- findall(pos(X, Y), (between(1, 3, X), between(1, 3, Y)), L1), findall(pos(X, Y), (between(1, 3, X), between(1, 3, Y)), L2), findall((P1, P2), (member(P1, L1), member(P2, L2), P1 \= P2, P1 @< P2, not(member(quantum(P1, P2, Color), State)), not(member(classic(P1, _), State)), not(member(classic(P2, _), State))), Pairs).
...
valid(quantum(P3, P4, C), P1, P2, Color) :- C == Color, P3 \== P1, P3\== P2, P4 \== P1, P4 \== P2.
valid(classic(P, C), P1, P2, Color) :- C == Color, P \== P1, P \== P2.
valid_moves(State, Color, Moves) :- valid_pairs(State, Color, Pairs), findall(move(L, quantum(P1, P2, Color)), (member(L, State), member((P1,P2), Pairs), valid(L, P1, P2, Color)), Moves). 
% ----------------------------------------------------------------------
...
% winner(+State, -Colors)
check_winner([]):- false.
check_winner([_]):- false.
check_winner([_, _]):- false.
check_winner([pos(X1,Y1), pos(X2,Y2), pos(X3,Y3)]) :- (X1 == X2, X1 == X3) ; (Y1 == Y2, Y1 == Y3) ; (X1 == Y1, X2 == Y2, X3 == Y3) ; (sumlist([X1,Y1], 4),  sumlist([X2,Y2], 4), sumlist([X3,Y3], 4)).
winner(State, Colors) :- findall(C, (member(C, [white, black]), findall(P, member(classic(P, C), State), L), check_winner(L)), Colors), length(Colors, N), N > 0 ; fail.
% ----------------------------------------------------------------------
